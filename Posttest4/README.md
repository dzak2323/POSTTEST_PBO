...
### 5. Polimorfisme
Program menerapkan dua jenis Polimorfisme sesuai standar Modul 5:
* **Statis (Overloading)**: Method `setStok()` pada kelas `bahanBaku` memiliki dua bentuk (satu parameter untuk audit total, dan dua parameter untuk penambahan/pengurangan stok).
* **Dinamis (Overriding)**: Method `getInstruksiPenyimpanan()` dan `tampilkanBahan()` didefinisikan ulang di subclass `BahanBasah` dan `BahanKering` untuk memberikan informasi penyimpanan yang spesifik bagi tiap jenis bahan.
